...
class commodity(object):
	def __init__(self, commodity_code = "", description = "", productline_suffix = "", indents = 0, leaf = 0):
		...
		self.parent_commodity_code		= ""
		self.parent_productline_suffix  = ""
		self.descriptions				= []
		self.combined_breadcrumb		= ""
	...
	def get_category(self):
		for c in app.categories:
			l = len(c.stem)
			if self.commodity_code[0:l] == c.stem:
				self.category = c.description
				#print ("Match", c.stem, self.commodity_code)
				break



	def combine_breadcrumb(self):
		count = len(self.descriptions)
		#print (count)
		if count > 0:
			for loop in range(count - 1, -1, -1):
				d = self.descriptions[loop]
				self.combined_breadcrumb += d + ", "
			self.combined_breadcrumb = self.combined_breadcrumb.strip()
			self.combined_breadcrumb = self.combined_breadcrumb.strip(",")


	def get_siblings(self, c, my_index):
		for loop1 in range(my_index - 1, -1, -1):
			prior_commodity = c.commodity_list[loop1]
			if prior_commodity.parent_commodity_code == self.parent_commodity_code:
				self.siblings.append(prior_commodity)
			if prior_commodity.indents < self.indents:
				break


	def get_breadcrumbs(self, c, my_index):
		self.descriptions.append (f.lower_first_char(self.original_description))
		
		self.my_cc			= self.commodity_code
		self.my_pls			= self.productline_suffix
		self.my_parent_cc	= self.parent_commodity_code
		self.my_parent_pls	= self.parent_productline_suffix

		while my_index > 0:
			my_index = self.get_breadcrumb(c, my_index)
		

	def get_breadcrumb(self, c, my_index):
		for loop1 in range(my_index - 1, -1, -1):
			prior_commodity = c.commodity_list[loop1]

			if prior_commodity.productline_suffix == self.my_parent_pls and prior_commodity.commodity_code == self.my_parent_cc:
				if prior_commodity.indents == 0 and prior_commodity.significant_digits == 2:
					self.descriptions.append(prior_commodity.original_description.capitalize())
				else:
					self.descriptions.append(f.lower_first_char(prior_commodity.original_description))
				
				self.my_cc			= prior_commodity.commodity_code
				self.my_pls			= prior_commodity.productline_suffix
				self.my_parent_cc	= prior_commodity.parent_commodity_code
				self.my_parent_pls	= prior_commodity.parent_productline_suffix

				ret = loop1
				break
			elif prior_commodity.indents < 1: # and prior_commodity.significant_digits == 2:
				ret = -1
				break
		return (ret)
	...
	def combine_duties(self):
		...
		self.combined_duty_safe = self.combined_duty
	...
	def format_description_safe(self):
		self.latinise()
		self.description = str(self.description)
		self.description = self.description.replace("|of the CN", "")
		self.description = self.description.replace("liters", "litres")
	...
	def format_commodity_code(self):
		...
		self.eight_digit = s[0:8]
		#self.get_friendly()


	def get_friendly(self):
		for item in app.friendly_names:
			if item.commodity_code == self.eight_digit:
				self.friendly = item.description
				if self.significant_digits == 10:
					#print (self.commodity_code, self.original_description)
					self.friendly += " - " + self.original_description + " ADDED 10 digits"
				break
	...
	def check_for_seasonal(self):
		...
		for s in app.seasonal_list:
			if self.commodity_code == s.commodity_code and self.productline_suffix == "80":
				seasonal_records += 1
