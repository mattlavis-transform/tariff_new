...
class chapter(object):
	...
	def format_chapter(self):
		...
		if app.document_type == "schedule":
			...
			# Take the opportunity to work out the parent / child relationship here
			# This is so we can get the breadcrumb of descriptions
			...
			for loop1 in range(0, commodity_count):
				my_commodity = self.commodity_list[loop1]
				if my_commodity.original_description == "Other":
					my_commodity.get_siblings(self, loop1)
					if len(my_commodity.siblings) == 1:
						sib = f.lower_first_char(my_commodity.siblings[0].original_description)
						my_commodity.original_description = "other than " + sib
					else:
						my_commodity.original_description = "other"
				my_commodity.get_friendly()

			"""
			# Get breadcrumb
			for loop1 in range(0, commodity_count):
				my_commodity = self.commodity_list[loop1]
				my_commodity.get_breadcrumbs(self, loop1)


			# Debug
			for c in self.commodity_list:
				c.combine_breadcrumb()
				print (c.commodity_code, c.productline_suffix, c.combined_breadcrumb)
			"""	
			#sys.exit()
						...
						if my_commodity.significant_digits == 4:
							if upper_commodity.significant_digits == 2:
								if upper_commodity.combined_duty != "":
									...
									my_commodity.combined_duty_safe = upper_commodity.combined_duty_safe
								break
						else:
					...
					if my_commodity.combined_duty != "":
						...
						if loop1 < commodity_count:
							for loop2 in range(loop1 + 1, commodity_count):
								next_commodity = self.commodity_list[loop2]
		...
		for my_commodity in self.commodity_list:
			if my_commodity.assigned == True:
				if my_commodity.combined_duty_safe not in ("0.0%", "", "AU"):
					app.worksheet_mfn.write("A" + str(app.mfn_excel_row_count), my_commodity.category, app.text_format)
					app.worksheet_mfn.write("B" + str(app.mfn_excel_row_count), my_commodity.commodity_code, app.text_format)
					app.worksheet_mfn.write("C" + str(app.mfn_excel_row_count), my_commodity.friendly, app.text_format)
					app.worksheet_mfn.write("E" + str(app.mfn_excel_row_count), my_commodity.combined_duty_safe, app.right_format)
					app.mfn_excel_row_count += 1
			...
			if my_commodity.suppress_row == False:
				if app.document_type == "schedule":
					...
					"""
					# Write to the regional trade agreement file
					if app.country_profile != "":
						if my_commodity.productline_suffix == "80":
							app.rta_excel_row_count += 1
							app.rta_write_cell('A', app.rta_excel_row_count, my_commodity.commodity_code_formatted, app.nowrap)
							app.rta_write_cell('B', app.rta_excel_row_count, my_commodity.original_description, app.indent_formats[my_commodity.indents])
							app.rta_write_cell('C', app.rta_excel_row_count, my_commodity.combined_duty_safe, app.nowrap)

							for c in app.commodity_list:
								if c.commodity_code == my_commodity.commodity_code:
									app.rta_write_cell('D', app.rta_excel_row_count, c.duty_string, app.nowrap)
									break
					"""
