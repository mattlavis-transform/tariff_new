...
class application(object):
    ...
    def get_master_nomenclature_list(self):
        ...
        for loop1 in range(0, list_count):
            ...
            if g.prevailing_measure is not None:
        ...
        for loop1 in range(0, list_count):
            ...
            if g.eea_measure is not None:
                out += str(g.eea_measure.combined_duty) + ","
            else:
                out += ","

            if g.eea_iceland_measure is not None:
                out += str(g.eea_iceland_measure.combined_duty) + ","
            else:
                out += ","

            if g.iceland_measure is not None:
                out += str(g.iceland_measure.combined_duty) + ","
            else:
                out += ","

            if g.norway_measure is not None:
    ...
    def get_preferences(self, geographical_area_id):
        ...
        if len(rows) > 0:
            for rw in rows:
                ...
                # sThis deals with EPS values where the rates are in measure condition components, not measures
                if duty_expression_id is None:
            ...
            for m in measure_list:
                if m.mark_for_measure_trimming is True:
