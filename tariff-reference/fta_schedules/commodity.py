	...
	def resolve_measures(self):
		...
		if is_all_full_year or is_infinite:
			...
			for m in self.measure_list:
				...
				self.duty_string_rta += m.combined_duty_rta
			...
			if measure_count == 1:
				...
				if m.extent in (365, 366, -1):
					...
					self.duty_string_rta	= m.excel_with_dates() + self.duty_string_rta
			else:
				for measure in self.measure_list:
					self.duty_string += measure.xml_with_dates()
					self.duty_string_rta	= measure.excel_with_dates() + self.duty_string_rta
